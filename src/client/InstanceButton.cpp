...
InstanceButton::InstanceButton(InstanceType instance, int x, int y) {
    ...
    _x = x;
    _y = y;
    _entities[formatText("instanceBackground")] =
        std::make_shared<EntitySFML>(ressourceManager);
    ...
    _texts[formatText("instanceGame")]->setPosition(
        sf::Vector2f(_x + 10, _y + 25));

    _entities[formatText("userIcon")] =
        std::make_shared<EntitySFML>(ressourceManager);
    ...
    std::string playersText = std::to_string(_instance.nbPlayers) + " / " +
                              std::to_string(_instance.maxPlayers);
    ...
    _texts[formatText("playersText")]->setPosition(
        sf::Vector2f(_x + 30, _y + 55));

    if (instance.nbPlayers < instance.maxPlayers) {
        _entities[formatText("submitButton")] =
            std::make_shared<EntitySFML>(ressourceManager);
        ...
        _texts[formatText("submitButtonText")]->setPosition(
            sf::Vector2f(_x + 275, _y + 55));
    }
}
