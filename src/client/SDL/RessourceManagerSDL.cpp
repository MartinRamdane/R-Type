...
RessourceManagerSDL::RessourceManager(SDL_Renderer* renderer) {
    ...
    for (const auto& entry : std::filesystem::recursive_directory_iterator(directory)) {
        if (entry.is_regular_file()) {
            ...
            key = key + ".png";
            SDL_Surface *imageSurface = IMG_Load(key.c_str());
...
std::map<std::string, std::shared_ptr<SDL_Texture>> RessourceManagerSDL::getTextures() const {
    return _textures;
}