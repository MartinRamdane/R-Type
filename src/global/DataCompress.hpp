...
class DataCompress {
   public:
    DataCompress(){};
    DataCompress(const char* src) { Compress(src); }
    DataCompress(int compressed_size, int og_size)
        : compressed_size(compressed_size), original_size(og_size){};
    ...
    char* Compress(const char* src) {
        ...
        compressed_size =
            LZ4_compress_default(src, compressed, src_size, maxCompressedSize);
    ...
    char* Decompress(const char* src) {
        ...
        const int decSize = LZ4_decompress_safe(src, decompressed,
    ...
    void setCompressedSize(int size) { compressed_size = size; }
    int getCompressedSize() { return compressed_size; }
    void setOriginalSize(int size) { original_size = size; }
    int getOriginalSize() { return original_size; }
    char* getCompressed() { return compressed; }
