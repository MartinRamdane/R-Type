...
std::vector<uint8_t> EventHandler::encodeMessage() {
    ...
    std::vector<uint8_t> data(sizeof(ACTION) + sizeof(int) + sizeof(int) +
                              event.compressed_size + 4 * sizeof(uint32_t));
    // Copy the event data into the vector
    std::memcpy(data.data(), &event.ACTION_NAME, sizeof(ACTION));
    std::memcpy(data.data() + sizeof(ACTION), &event.original_size,
                sizeof(int));
    std::memcpy(data.data() + sizeof(ACTION) + sizeof(int),
                &event.compressed_size, sizeof(int));
    std::memcpy(data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int),
                event.body, event.compressed_size);
    std::memcpy(data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) +
                    event.compressed_size,
                &bodyCrc, sizeof(uint32_t));
    std::memcpy(data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) +
                    event.compressed_size + sizeof(uint32_t),
                &compressedCrc, sizeof(uint32_t));
    std::memcpy(data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) +
                    event.compressed_size + 2 * sizeof(uint32_t),
                &originalCrc, sizeof(uint32_t));
    std::memcpy(data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) +
                    event.compressed_size + 3 * sizeof(uint32_t),
                &actionCrc, sizeof(uint32_t));
...
Event EventHandler::decodeMessage(std::vector<uint8_t> data) {
    try {
        ...
        if (data.size() < sizeof(ACTION) + 2 * sizeof(int) +
                              netevt.compressed_size + 4 * sizeof(uint32_t)) {
            throw std::runtime_error(
                "[ERROR] Packet has been corrupted. Not enought size for the "
                "whole "
                "CRC");
        }
        std::memcpy(&netevt.ACTION_NAME, data.data(), sizeof(ACTION));
        std::memcpy(&netevt.original_size, data.data() + sizeof(ACTION),
                    sizeof(int));
        std::memcpy(&netevt.compressed_size,
                    data.data() + sizeof(ACTION) + sizeof(int), sizeof(int));
        std::memcpy(&bodyCrc,
                    data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) +
                        netevt.compressed_size,
                    sizeof(uint32_t));
        std::memcpy(&compressedCrc,
                    data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) +
                        netevt.compressed_size + sizeof(uint32_t),
                    sizeof(uint32_t));
        std::memcpy(&originalCrc,
                    data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) +
                        netevt.compressed_size + 2 * sizeof(uint32_t),
                    sizeof(uint32_t));
        std::memcpy(&actionCrc,
                    data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) +
                        netevt.compressed_size + 3 * sizeof(uint32_t),
                    sizeof(uint32_t));
        if (compressedCrc != calculateCRCForInt(netevt.compressed_size)) {
            throw std::runtime_error(
                "[ERROR] Packet has been corrupted. CRC is not valid for "
                "compressed "
                "size");
        }
        ...
        uint32_t compressedCRC =
            calculateCRCForBody(netevt.body, netevt.compressed_size);
        if (compressedCRC != bodyCrc) {
            delete[] netevt.body;
            throw std::runtime_error(
                "Packet has been corrupted. CRC is not valid for body");
        }
        char* decompressed = decompressor.Decompress(netevt.body);
        decompressed[decompressor.getOriginalSize() - 1] = '\0';
        if ((int)strlen(decompressed) !=
            (int)decompressor.getOriginalSize() - 1) {
            delete[] decompressed;
            delete[] netevt.body;
