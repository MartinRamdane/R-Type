...
std::vector<uint8_t> EventHandler::encodeMessage() {
    ...
    std::memcpy(data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int), event.body,
                event.compressed_size);
    std::memcpy(data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) + event.compressed_size,
...
Event EventHandler::decodeMessage(std::vector<uint8_t> data) {
    try {
        ...
        if (data.size() <
            sizeof(ACTION) + 2 * sizeof(int) + netevt.compressed_size + 4 * sizeof(uint32_t)) {
            throw std::runtime_error(
                "[ERROR] Packet has been corrupted. Not enought size for the whole CRC");
        }
        std::memcpy(&netevt.ACTION_NAME, data.data(), sizeof(ACTION));
        std::memcpy(&netevt.original_size, data.data() + sizeof(ACTION), sizeof(int));
        std::memcpy(&netevt.compressed_size, data.data() + sizeof(ACTION) + sizeof(int),
                    sizeof(int));
        std::memcpy(
            &bodyCrc,
            data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int) + netevt.compressed_size,
            sizeof(uint32_t));
        ...
        if (compressedCrc != calculateCRCForInt(netevt.compressed_size)) {
            throw std::runtime_error(
                "[ERROR] Packet has been corrupted. CRC is not valid for compressed size");
        }
        if (originalCrc != calculateCRCForInt(netevt.original_size))
            throw std::runtime_error(
                "[ERROR] Packet has been corrupted. CRC is not valid for original size");
        if (actionCrc != calculateCRCForAction(netevt.ACTION_NAME))
            throw std::runtime_error(
                "[ERROR] Packet has been corrupted. CRC is not valid for action");
        ...
        std::memcpy(netevt.body, data.data() + sizeof(ACTION) + sizeof(int) + sizeof(int),
