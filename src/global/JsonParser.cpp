...
nlohmann::json JsonParser::readFile(std::string const& fileName) {
            ...
            if (entry.is_regular_file()) {
                ...
                if (key == fileName) {
                    ...
                    if (inputFile.is_open()) {
                        ...
                        std::cerr << "Impossible de trouver le JSON : " << key
                                  << std::endl;
    ...
    std::cerr << "Here Impossible de trouver le JSON : " << fileName
              << std::endl;
    // You might want to throw an exception or return a default JSON object.
    return {};
}

std::vector<std::string> JsonParser::split(std::string const& str,
                                           char const delim) noexcept {
...
std::unordered_map<std::string, std::string> JsonParser::getJsonFile(
