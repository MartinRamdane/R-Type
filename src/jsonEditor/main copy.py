...
def recursive_json_reader(jsonFile, window, row_counter):
    if isinstance(jsonFile, dict):
        ...
        for element in jsonFile:
            # window.extend_layout(window['-ROW_PANEL-'], [create_simple_row(row_counter, element)])
            recursive_json_reader(element, window, row_counter)
    else:
...
def main(jsonList):
    ...
    while True:
        ...
        if event == '-MODIFY-':
            print(values['-MODIFY-'])
            # window['-STATUS-'].update(values['-MODIFY-'])
    window.close()
