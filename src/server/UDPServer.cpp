...
UDPServer::UDPServer(boost::asio::io_service &io_service, int port) : socket_(io_service,
...
void UDPServer::processSendQueue() {
    socket_.async_send_to(boost::asio::buffer(_toSendQueue.front().data), _toSendQueue.front().endpoint,
