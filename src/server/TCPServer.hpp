...
class TCPServer {
   public:
    TCPServer(int port)
        : _acceptor(io_context_, tcp::endpoint(tcp::v4(), port)) {}
            ...
            [this](const boost::system::error_code& error, tcp::socket socket) {
                if (!error) {
                    std::shared_ptr<TCPConnection<T>> new_connection =
                        std::make_shared<TCPConnection<T>>(
                            TCPConnection<T>::owner::server, io_context_,
                    ...
                    std::cout << "[ERROR TCP Server]: " << error.message()
                              << std::endl;
                }
                WaitForClientAsync();
            });
    }
    void SendMessageAsync(std::shared_ptr<TCPConnection<T>> client,
                          const message<T>& msg) {
   ...
   protected:
    ...
    virtual void OnMessage(std::shared_ptr<TCPConnection<T>> client,
                           message<T>& msg) {
        std::cout << "Client :" << client->GetID()
                  << " read: " << msg.body.size() << std::endl;
    }
