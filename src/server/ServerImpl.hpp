...
class MyServer : public TCPServer<ACTION> {
   public:
    MyServer(int port, ServerClass* _server)
        : TCPServer<ACTION>(port), _server(_server) {}
   ...
   protected:
    virtual void OnMessage(std::shared_ptr<TCPConnection<ACTION>> client,
                           message<ACTION>& msg) {
        switch (msg.header.id) {
            case ACTION::CREATE: {
                try {
                    ...
                    Instance* instance =
                        _server->getInstance(_server->getInstancesNb() - 1);
                    Event evt;
                    ...
                    instance->getUDPServer()->addPlayerEntity(playerId,
                                                              playerEntityId);
                    instance->setPlayerIdToGive(instance->getPlayerIdToGive() +
                                                1);
            ...
            case ACTION::LIST: {
                ...
                case ACTION::JOIN: {
                    try {
                        ...
                        ss >> port;
                        Instance* instance =
                            _server->getInstanceByPort(std::stoi(port));
                        if (instance == nullptr) {
                            std::cerr << "[ERROR]: Bad instance to join"
                                      << std::endl;
                            return;
                        }
                        Event evt;
                        ...
                        int playerId = instance->getPlayerIdToGive();
                        evt.body =
                            std::to_string(instance->getPlayerIdToGive()) +
                            " " + port;
                        std::string playerEntityId =
                            "p" + std::to_string(playerId);
                        instance->getUDPServer()->addPlayerEntity(
                            playerId, playerEntityId);
                        instance->setPlayerIdToGive(
                            instance->getPlayerIdToGive() + 1);
                        SendEvent(client, evt);
                    } catch (std::exception& e) {}
                } break;
                case ACTION::JOINED: {
                    std::cout << "[" << client->GetID() << "]: JOINED"
                              << std::endl;
                } break;
                case ACTION::READY: {
                    std::cout << "[" << client->GetID() << "]: READY"
                              << std::endl;
                } break;
                case ACTION::LEFT: {
                    std::cout << "[" << client->GetID() << "]: LEFT"
                              << std::endl;
                } break;
                case ACTION::RIGHT: {
                    std::cout << "[" << client->GetID() << "]: RIGHT"
                              << std::endl;
                } break;
                case ACTION::UP: {
                ...
                case ACTION::DOWN: {
                    std::cout << "[" << client->GetID() << "]: DOWN"
                              << std::endl;
                } break;
                case ACTION::SPACE: {
                    std::cout << "[" << client->GetID() << "]: SPACE"
                              << std::endl;
                } break;
                case ACTION::QUIT: {
                    std::cout << "[" << client->GetID() << "]: QUIT"
                              << std::endl;
                    ...
                    if (instance == nullptr) {
                        std::cerr << "[ERROR]: Bad instance to quit"
                                  << std::endl;
                        return;
                    }
                ...
                case ACTION::PONG: {
                    std::cout << "[" << client->GetID() << "]: PONG"
                              << std::endl;
                } break;
                case ACTION::UNKNOWN: {
                    std::cout << "[" << client->GetID() << "]: UNKNOWN"
                              << std::endl;
                } break;
                case ACTION::SPRITE: {
                    std::cout << "[" << client->GetID() << "]: SPRITE"
                              << std::endl;
                } break;
                case ACTION::DEAD: {
                    std::cout << "[" << client->GetID() << "]: DEAD"
                              << std::endl;
                }
                case ACTION::FLIP: {
                    std::cout << "[" << client->GetID() << "]: FLIP"
                              << std::endl;
                }
                case ACTION::TEXT: {
                    std::cout << "[" << client->GetID() << "]: TEXT"
                              << std::endl;
                }
                case ACTION::RESET: {
                    std::cout << "[" << client->GetID() << "]: RESET"
                              << std::endl;
                }
                case ACTION::CHECK: {
                    std::cout << "[" << client->GetID() << "]: CHECK"
                              << std::endl;
                }
                case ACTION::SOUND: {
                    std::cout << "[" << client->GetID() << "]: SOUND"
                              << std::endl;
                }
                default:
                    break;
